...
export const getAnnonceByrec = createAsyncThunk(
    "annonces/getAnnonceByrec",
    async(id, thunkAPI) => {
        try {
            const user = thunkAPI.getState().auth.user;
            return await annonceService.getAnnonceByrec(user._id);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const annonceSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getAnnonceByrec.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(getAnnonceByrec.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.annonces = action.payload;
