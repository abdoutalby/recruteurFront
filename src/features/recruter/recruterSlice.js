...
export const updateRecruter = createAsyncThunk(
    "recruters/update",
    async(id, thunkAPI) => {
        try {
            const token = thunkAPI.getState().auth.user.token;
            return await recruterService.updateRecruter(id, token);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const recruterSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateRecruter.pending, (state) => {
                state.recrruterIsLoading = true;
            })
            .addCase(updateRecruter.fulfilled, (state, action) => {
                state.recrruterIsLoading = false;
                state.recruterIsSuccess = true;
                state.recruters = action.payload;
            })
            .addCase(updateRecruter.rejected, (state, action) => {
                state.recrruterIsLoading = false;
                state.recruterIsError = true;
                state.recruterMessage = action.payload;
            })
